...
const loadRoomInfo = async () => {
  try {
    ...
    if (response && response.room) {
      Object.assign(roomInfo, response.room);
      // 检查当前用户是否在房间内
...
const joinRoom = async () => {
  try {
    ...
    if (response && response.success) {
      ...
      showToast(response?.message || '加入房间失败');
    }
  } catch (error) {
  ...
  showConfirmDialog({
    ...
    try {
      ...
      if (response && response.success) {
        ...
        showToast(response?.message || '离开房间失败');
      }
    } catch (error) {
  ...
  showConfirmDialog({
    ...
    try {
      ...
      if (response && response.success) {
        ...
        showToast(response?.message || '踢出失败');
      }
    } catch (error) {
...
const startGame = async () => {
  try {
    ...
    if (response && response.success) {
      ...
      showToast(response?.message || '开始游戏失败');
    }
  } catch (error) {
...
const sendMessage = async () => {
  ...
  try {
    // 发送到服务器
    await roomApi.sendMessage(roomId, chatInput.value.trim());
  } catch (error) {
...
onMounted(() => {
  loadRoomInfo();
  // 可以添加定时刷新房间信息的逻辑
  const refreshInterval = setInterval(loadRoomInfo, 5000);
